...
const signUp=(datos)=> fetch(`${CONSTANTES.URLAPI}/eos/sign-up/`,
{
  method: 'POST',
  body:JSON.stringify(datos),
  headers: new Headers({
...
function* sagasSignIn(values){
  try {
    console.log(values.data)
    const user= yield call (signUp, values.data)
    if(user.token){
      localStorage.setItem('user', user.token);
      yield put (userActions.loginSuccess(user))
      history.push('/account')
...
export default function* sagasPrimary() {
  ...
  yield takeEvery(CONSTANTES.SIGN_UP_REQUEST, sagasSignIn)
